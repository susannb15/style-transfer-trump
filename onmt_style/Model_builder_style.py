...
def load_test_model(opt, model_path=None):
    ...
    model_opt = onmt_stlye.ArgumentParser.ckpt_model_opts(checkpoint['opt'])
    onmt_style.ArgumentParser.update_model_opts(model_opt)
    onmt_style.ArgumentParser.validate_model_opts(model_opt)
...
def build_task_specific_model(model_opt, fields):
    ...
    if model_opt.model_task == ModelTask.SEQ2SEQ:
        try:
            enc_check = torch.load(model_opt.encoder, map_location=lambda storage, loc: storage)
            encoder_opts = enc_check['opt']
            src_dict= enc_check['vocab']['src']
            encoder  = build_encoder(encoder_opts, src_dict)
        ...
        model = onmt_style.StyleDecoder(decoder=decoder)
...
def build_base_model(model_opt, opt, fields, gpu, checkpoint=None, gpu_id=None):
    ...
    if checkpoint is not None:
        ...
        checkpoint['encoder'] = {fix_key(k): v
                               for k, v in checkpoint['encoder'].items()}
        checkpoint['decoder'] = {fix_key(k): v
    ...
    # Build classifier
    #classifier = AutoModelForSequenceClassification.from_pretrained(opt.classifier, num_labels=2)
    classifier_check = torch.load(opt.classifier, map_location=lambda storage, loc:storage)
    classifier_opt = classifier_check['opt']
    classifier_dict = classifier_check['dicts']['src']
    classifier = onmt_style.ConvNet(classifier_opt, classifier_dict)
    classifier.load_state_dict(classifier_check['model'])

    model.classifier = classifier
    class_input = nn.Sequential(nn.Linear(opt.rnn_size, classifier_dict.size()))
