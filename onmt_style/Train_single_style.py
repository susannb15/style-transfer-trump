...
def main(opt, fields, transforms_cls, checkpoint, device_id,
    ...
    # Build model.
    model = onmt_style.build_model_style(model_opt, opt, fields, checkpoint)
    ...
    # Build model saver
    model_saver = onmt_style.build_model_saver(model_opt, opt, model, fields, optim)

    # Targets for the classifier (Trump = 1)
    class_targets = opt.classifier_targets

    trainer = onmt_style.build_trainer(
        opt, device_id, model, class_targets, fields, optim, model_saver=model_saver)
