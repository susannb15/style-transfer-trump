...
def _init_train(opt):
    ...
    onmt_style.ArgumentParser.validate_prepare_opts(opt)

    if opt.train_from:
        # Load checkpoint if we resume from a previous training.
        checkpoint = onmt_style.load_checkpoint(ckpt_path=opt.train_from)
...
def train(opt):
    ...
    onmt_style.ArgumentParser.validate_train_opts(opt)
    onmt_style.ArgumentParser.update_model_opts(opt)
    onmt_style.ArgumentParser.validate_model_opts(opt)

    set_random_seed(opt.seed, False)

    checkpoint, fields, transforms_cls = _init_train(opt)
    train_process = partial(
        onmt_style.single_main_style,
        fields=fields,
    ...
    if opt.world_size > 1:
        ...
        for device_id in range(nb_gpu):
            ...
            train_iter = onmt_style._build_train_iter_style(
...
def _get_parser():
    parser = onmt_style.ArgumentParser(description='train.py')
    onmt_style.train_opts(parser)
    return parser
