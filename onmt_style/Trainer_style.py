...
def build_trainer(opt, device_id, model, classifier_targets, fields, optim, model_saver=None):
    """
    ...
    train_loss = onmt_style.build_loss_compute(model, tgt_field, opt)
    valid_loss = onmt_style.build_loss_compute(model, tgt_field, opt, train=False)
    ...
    trainer = Trainer(model, classifier_targets, train_loss, valid_loss, optim, trunc_size,
...
class Trainer(object):
    ...
    def __init__(self, model, classifier_targets, train_loss, valid_loss, optim,
        ...
        # Basic attributes.
        self.model = model
        self.classifier_targets = classifier_targets
        ...
        with torch.no_grad():
            ...
            for batch in valid_iter:
                ...
                with torch.cuda.amp.autocast(enabled=self.optim.amp):
                    ...
                    enc_hidden, context, lengths = self.model.encoder(src, src_lengths)
                    outputs, attns = valid_model(
                        src, tgt, lengths, enc_hidden, context, with_align=self.with_align)
            ...
            for j in range(0, target_size - 1, trunc_size):
                ...
                with torch.cuda.amp.autocast(enabled=self.optim.amp):
                    ...
                    outputs, attns = self.model(
                        src, tgt, lengths, enc_hidden, context, bptt=bptt, with_align=self.with_align)
                    bptt = True
